...
Tool mapping: `execute_luau` → MCP `mcp__Roblox_Studio__execute_luau` (or `rodeo run`); `screen_capture` → MCP `mcp__Roblox_Studio__screen_capture`; `get_console_output` → MCP `mcp__Roblox_Studio__get_console_output`. `DeviceSimulatorService` calls run in the **edit DM**; verify with `screen_capture`, and use `/device-playtest` for runtime-created UI. `commands/` are the four test routines; `API_spec.md` is the service reference.
