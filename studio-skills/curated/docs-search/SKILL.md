...
## Execution in this repo (Studio MCP / WebFetch)

`http_get` → MCP `mcp__Roblox_Studio__http_get` (or `mcp__rodeo__http_get`), which supports the `query` / `context_lines` / `return_full` params below and enforces the `.md`/`llms.txt` rule — use it for cheap multi-page keyword checks. No play mode needed. When Studio isn't connected, plain `WebFetch` against the same `create.roblox.com/.../<Name>.md` URLs returns full docs (without the cheap query-miss behavior).
